...
var sendNewVoteMessage = function (to, from) {
  if (state.current.servers[from-1].state !== 'candidate') {
    state.current.servers[from-1].state = 'candidate'
  }
  var message = {
    from: from,
    to: to,
...
var becomeLeader = function (serverIndex) {
  state.current.servers[serverIndex].state = "leader"
  state.current.servers[serverIndex].electionAlarm = util.Inf;
}

var grantVote = function (serverIndex, voteFrom) {
  // voteGranted starts with 1
  state.current.servers[serverIndex].voteGranted[voteFrom] = true
}
...
function connect(url, index) {
  ...
  newWS.onmessage = function(message) {
    ...
    switch (parsedMessage.action) {
      case 1:
        serverResetElectionTimeout(parsedMessage.serverIndex, parsedMessage.timeoutLength);
        break;
      case 2:
      ...
      case 8:
        ...
        break;
      case 9:
        grantVote(parsedMessage.serverIndex, parsedMessage.from);
        break;
      case 10:
